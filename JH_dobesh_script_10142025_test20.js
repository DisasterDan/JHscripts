...
if (
    ...
    if (mainBackground) {
        ...
        const updateSwiperMargin = () => {
            ...
            if (bannerSwiper) {
                bannerSwiper.style.marginTop = `${mainBackgroundHeight - 150}px`;
            }
        };
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateMarginTop() {
        ...
        if (isWide) {
            const mainBackgroundHeight = mainBackground.getBoundingClientRect().height;
            bannerSwiper.style.setProperty('margin-top', `${Math.ceil(mainBackgroundHeight)}px`, 'important');
        } else {
