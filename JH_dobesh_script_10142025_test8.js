...
if (
    ...
    if (mainBackground) {
        ...
        const swiperHTML = `
            <div class="banner-swiper" style="margin-top: 100vh;margin-bottom: 60px;visibility: hidden;">
        ...
        bannerSwiper.style.setProperty('visibility', 'visible');
        
        function updateSwiperMargin() {
            const mainBackground = document.querySelector('#main-background');
            if (!mainBackground || !bannerSwiper) return;
            const h = mainBackground.offsetHeight || window.innerHeight;
            bannerSwiper.style.setProperty('margin-top', `${Math.max(h - 150, 0)}px`, 'important');
        }
